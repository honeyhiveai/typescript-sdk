 ...
 */
export async function eventsCreateModelEvent(
...
> {
    ...
    const parsed$ = schemas$.safeParse(
        input$,
        (value$) => operations.CreateModelEventRequestBody$outboundSchema.parse(value$),
    ...
    const path$ = pathToFunc("/events/model")();
    ...
    const [result$] = await m$.match<
        ...
        | ConnectionError
    >(
        m$.json(200, operations.CreateModelEventResponseBody$inboundSchema),
        m$.fail(["4XX", "5XX"])
    )(response);
    if (!result$.ok) {
        return result$;
